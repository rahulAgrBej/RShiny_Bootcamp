...
title: 6.0 Connecting UI and Server
sidebar_position: 6
---
...
Now that we have understood how to make our UI interactive, and separately understood how to make our graphs its time to connect the two parts of our web application (our server and UI).
...
Next we need to reference this function in our server. We will call it within a pair of ```({ })```. This is itself is a function that does nothing, since it has nothing inside. However this ```({ getData('data/lead-tool-chart-data.csv') })``` is a function that calls our getData function. In our server we have to wrap all outputs in some sort of function, ie everything has to be inside some sort of ```({ })```.

Note that in our particular case we placed our getData inside of a ```reactive()``` function. This reactive function lets us load the data only once and store its value locally. This way our web application works efficiently and doesn't load and clean our data file everytime it is needed. Instead it loads our data once, keeps a local copy, that then gets referenced whenever it is needed. The reactive function will only load our data again if it changes. This can be quite useful especially when our data files can be quite large and unaltered. 
...
Now that we have data loaded into our server we can start to use it. The main way our server can send and receive data from and to the UI, is through the ```input``` and ```output``` variables.
...
Similarly every piece of data that we send from the server to the UI is available through the ```output``` variable. Therefore when we send this data over to the UI we have to save it under the ```output``` variable. For example, let's say we want to send over a bar chart plot we create, we would do so by saving it to ```output$energyBurdenBarChartID```, where ```energyBurdenBarChartID``` is the unique identifier that we have now associated with our bar chart.

## Sending a Custom Bar Chart from Server to UI

Now let's send over our custom bar chart to our UI. We have already defined a function that takes in data and a variable (user selected) and outputs a bar chart in our test.R. We'll be transfering this function to our server.R file.
...
Now we have a server file that has data loaded into it and a function that will transform that data, with a user selection, into a bar chart. Let's zoom into our server function now. We have to be specific about which function to use for our output, the same we choose to use the ```reactive({})``` wrapper function to load in our data. For all plots we will be using ```renderPlot({})``` and place our plot creating function ```energyBurdenBarChart``` inside.

Note: when referencing data that usees the ```reactive({})``` function we have to call it in order to use it. Therefore to reference our data we use ```dataInput()```. We don't have to save it or use it as ```output$dataInput()``` because our data is only used for internal server purposes and so the UI doesn't need to know about it.
...
## Recieving bar chart and displaying it in the UI
